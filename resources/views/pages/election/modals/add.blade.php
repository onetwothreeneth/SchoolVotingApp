<div class="modal fade" id="add" tabindex="-1" role="dialog" aria-hidden="true" data-modal="scroll">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="{{ URL::route('election.add') }}" method="post">
				{{ csrf_field() }}
				<div class="modal-body"> 
					...
					<div class="form-group">
						...
						<input type="date" class="form-control" name="voting_start" required min="{{ date('Y-m-d',strtotime(Carbon\Carbon::now('Asia/Singapore'))) }}">
					</div>   
					<div class="form-group">
						<label for="demoTextInput1">Voting End</label>
						<input type="date" class="form-control" name="voting_end" required min="{{ date('Y-m-d',strtotime(Carbon\Carbon::now('Asia/Singapore'))) }}">
					</div>   
				</div>
